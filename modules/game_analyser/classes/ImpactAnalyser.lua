...
function onImpactExtra(mousePosition)
	...
	menu:addOption(tr('Reset All-Time High'),
		function()
			ImpactAnalyser:setAllTimeHightDps(0); ImpactAnalyser:setAllTimeHightHps(0)
		end)
...
function ImpactAnalyser:checkAnchos()
	...
	if ImpactAnalyser.window.contentsPanel.dpsGraphBG:isVisible() then
		ImpactAnalyser.window.contentsPanel.damageTypeLabel:addAnchor(AnchorTop, 'separatorGraphHorizontalDps',
			AnchorBottom)
