...
function onLootingExtra(mousePosition)
	...
	menu:addCheckBox(tr('Loot Per Hour Gauge'), gaugeVisible,
		function() LootAnalyser:setLootPerHourGauge(not gaugeVisible) end)
	menu:addCheckBox(tr('Loot Per Hour Graph'), graphVisible,
		function() LootAnalyser:setLootPerHourGraph(not graphVisible) end)
...
function LootAnalyser:updateWindow(updateScroll, ignoreVisible)
	...
	if LootAnalyser.target == 0 and LootAnalyser.goldHour == 0 then
		...
		LootAnalyser.window.contentsPanel.lootTargetBG.lootArrow:setMarginLeft(math.min(targetMaxMargin,
			math.ceil(percent)))
	end

	LootAnalyser.window.contentsPanel.lootTargetBG:setTooltip(string.format("Current: %d\nTarget: %d",
		LootAnalyser.goldHour, LootAnalyser.target))
	...
	if table.empty(LootAnalyser.lootedItems) and #contentsPanel.lootedItems:getChildren() then
		...
		for itemId, info in pairs(LootAnalyser.lootedItems) do
			...
			widget:setTooltip(string.format("%s (Count: %s, Value: %dgp, Sum: %dgp)", string.capitalize(info.name),
				formatLargeNumber(info.count), info.basePrice, info.basePrice * info.count))
