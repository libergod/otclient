...
function PartyHuntAnalyser:onPartyAnalyzer(startTime, leaderID, lootType, membersData, membersName)
	...
	if not membersData or not next(membersData) or not membersName or #membersName == 0 then
		...
		if localPlayer then
			...
			local hadRecentData = PartyHuntAnalyser.hadRecentPartyData and
			(timeSinceLastData < 10000)                                                       -- within 10 seconds
...
function onPartyHuntExtra(mousePosition)
	...
	local isLeaderShield = table.contains({ ShieldYellow, ShieldYellowSharedExp, ShieldYellowNoSharedExpBlink },
		playerShield)
...
function PartyHuntAnalyser:clipboardData()
	...
	if session == 0 then
		...
		final = "Session data: From " ..
		os.date('%Y-%m-%d, %H:%M:%S', session) .. " to " .. os.date('%Y-%m-%d, %H:%M:%S') .. "\n"
